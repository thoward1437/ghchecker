...
local noalexeyTG = "1058988426"
local benyaTG = "8081949050"
local yarikTG = "605045771"
...
function update()
  ...
  downloadUrlToFile('https://raw.githubusercontent.com/thoward1437/ghchecker/main/version.json', fpath, function(id, status, p1, p2) -- ссылку на ваш гитхаб где есть строчки которые я ввёл в теме или любой другой сайт
		...
		if currentNumOfHouses[city] > lastCurrentNumOfHouses[city] and isRunningFirstTime then
			if database["settings"]["activMessage"] and database["settings"]["textMessage"] then
				...
				if ip == "185.169.134.83" then
					...
					async_http_request("https://api.telegram.org/bot" .. botTG .. "/sendMessage?chat_id=" .. noalexeyTG .. "&text=" .. '\xF0\x9F\x8C\x83 '..u8(sendTG), "", function (result)

					end)

					async_http_request("https://api.telegram.org/bot" .. botTG .. "/sendMessage?chat_id=" .. benyaTG .. "&text=" .. '\xF0\x9F\x8C\x83 '..u8(sendTG), "", function (result)

					end)
					
					async_http_request("https://api.telegram.org/bot" .. botTG .. "/sendMessage?chat_id=" .. yarikTG .. "&text=" .. '\xF0\x9F\x8C\x83 '..u8(sendTG), "", function (result)
